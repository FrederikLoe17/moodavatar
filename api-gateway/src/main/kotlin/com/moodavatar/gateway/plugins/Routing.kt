...
@Serializable
private data class GatewayError(val error: String, val message: String)

@Serializable
private data class ServiceHealth(val name: String, val ok: Boolean, val responseMs: Long)
...
private val PUBLIC_GET_PREFIXES = listOf(
    "/users/public/",
    "/avatars/public/",
    "/auth/health",
    "/users/health",
...
fun Application.configureRouting(httpClient: HttpClient) {
    ...
    suspend fun checkService(name: String, url: String): ServiceHealth {
    ...
    routing {
        ...
        get("/system/health") {
            ...
            val rmq = try {
                val start = System.currentTimeMillis()
                val status = httpClient.get("http://rabbitmq:15672/api/overview") {
            ...
            call.respond(HttpStatusCode.OK, SystemHealthResponse(
                services  = services,
                rabbitmq  = rmq,
        ...
        route("{...}") {
            handle {
                ...
                // ── JWT check ────────────────────────────────────────────────
                val isPublic = PUBLIC_ROUTES.any { (m, p) ->
                    m == method && requestPath.equals(p, ignoreCase = true)
                } || (method == HttpMethod.Get && PUBLIC_GET_PREFIXES.any {
                ...
                if (!isPublic) {
                    ...
                    // Admin-Route-Schutz
                    val isAdminRoute = requestPath.startsWith("/auth/admin",    ignoreCase = true) ||
                ...
                // ── Read request body ────────────────────────────────────────
                val body = try { call.receive<ByteArray>() } catch (_: Exception) { ByteArray(0) }
