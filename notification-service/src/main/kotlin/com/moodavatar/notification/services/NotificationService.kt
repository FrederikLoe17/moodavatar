...
class NotificationService {
    ...
    fun listForUser(userId: String, limit: Int = 50): List<NotificationDto> = transaction {
        Notifications
    ...
    fun unreadCount(userId: String): Int = transaction {
        Notifications
            .select { (Notifications.userId eq UUID.fromString(userId)) and (Notifications.read eq false) }
            .count().toInt()
    }

    fun markRead(id: String, userId: String): Boolean = transaction {
    ...
    fun markAllRead(userId: String) = transaction {
        Notifications.update({
            (Notifications.userId eq UUID.fromString(userId)) and (Notifications.read eq false)
        }) {
            it[read] = true
        }
    }
