...
class NeedsServiceTest {
    ...
    @Test
    fun `getNeeds returns 30 for all stats when no record exists`() = runBlocking {
        stubFind(null)
    ...
    @Test
    fun `getNeeds returns 100 for mood and energy right after mood checkin`() = runBlocking {
    ...
    @Test
    fun `getNeeds returns 0 for mood after 20 hours since last checkin`() = runBlocking {
    ...
    @Test
    fun `getNeeds returns 100 for social right after social event`() = runBlocking {
        val now = Instant.now().toString()
    ...
    @Test
    fun `getNeeds returns 0 for social after 40 hours since last social event`() = runBlocking {
    ...
    @Test
    fun `getNeeds returns 100 for activity right after activity event`() = runBlocking {
        val now = Instant.now().toString()
    ...
    @Test
    fun `getNeeds clamps all values between 0 and 100`() = runBlocking {
    ...
    @Test
    fun `onMoodCheckin calls updateOne on the collection`() = runBlocking {
    ...
    @Test
    fun `onSocialEvent calls updateOne on the collection`() = runBlocking {
        every { mockCollection.updateOne(any<Document>(), any<Document>(), any()) } returns mockk()
