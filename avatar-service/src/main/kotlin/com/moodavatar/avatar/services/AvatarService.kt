...
class AvatarService(private val db: MongoDatabase) {
    ...
    suspend fun setMood(userId: String, emotion: String, intensity: Int, note: String?): AvatarResponse =
        withContext(Dispatchers.IO) {
            ...
            fun list(key: String): List<String> =
                (existingConfig?.get(key) as? List<*>)?.filterIsInstance<String>() ?: emptyList()
    ...
    suspend fun getAdminStats(): MoodAdminStatsResponse = withContext(Dispatchers.IO) {
    ...
    private fun Document.toAvatarResponse(): AvatarResponse {
        ...
        @Suppress("UNCHECKED_CAST")
        fun list(key: String): List<String> =
            (config?.get(key) as? List<*>)?.filterIsInstance<String>() ?: emptyList()
