...
class NeedsService(private val db: MongoDatabase) {
    private val col = db.getCollection("avatar_needs")

    suspend fun getNeeds(userId: String): NeedsResponse = withContext(Dispatchers.IO) {
        val doc = col.find(Document("_id", userId)).firstOrNull()
        compute(doc)
    }

    suspend fun onMoodCheckin(userId: String) = withContext(Dispatchers.IO) {
    ...
    suspend fun onSocialEvent(userId: String) = withContext(Dispatchers.IO) {
        val now = Instant.now().toString()
        col.updateOne(
            Document("_id", userId),
