...
private fun ApplicationCall.isAdmin(): Boolean = principal<JWTPrincipal>()?.payload?.getClaim("role")?.asString() == "ADMIN"
...
fun Route.adminRoutes(adminService: AdminService) {
    route("/auth/admin") {
        authenticate("auth-jwt") {
            ...
            // GET /auth/admin/users?page=1&pageSize=20&search=
            get("/users") {
                if (!call.isAdmin()) return@get call.respond(HttpStatusCode.Forbidden, FORBIDDEN)
                val page =
            ...
            put("/users/{id}/role") {
                ...
                val role =
                    runCatching { Role.valueOf(req.role.uppercase()) }.getOrNull()
