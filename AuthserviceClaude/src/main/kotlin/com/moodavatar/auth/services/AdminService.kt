...
class AdminService {
    ...
    fun listUsers(page: Int, pageSize: Int, search: String?): AdminUsersResponse = transaction {
    ...
    fun updateRole(userId: UUID, role: Role): AdminUserResponse? = transaction {
        val updated = Users.update({ Users.id eq userId }) {
            it[Users.role]      = role
            it[Users.updatedAt] = LocalDateTime.now()
        }
        if (updated == 0) null
        else Users.select { Users.id eq userId }.single().toAdminResponse()
    }

    private fun ResultRow.toAdminResponse() = AdminUserResponse(
