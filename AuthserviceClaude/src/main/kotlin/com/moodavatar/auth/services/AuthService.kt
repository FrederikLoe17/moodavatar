...
class AuthService(private val cfg: ApplicationConfig) {
    ...
    // ── Registrierung ────────────────────────────────────
    fun register(req: RegisterRequest): UserResponse = transaction {
        if (Users.select { Users.email eq req.email }.count() > 0)
            error("EMAIL_TAKEN")
        if (Users.select { Users.username eq req.username }.count() > 0)
            error("USERNAME_TAKEN")

        val role  = runCatching { Role.valueOf(req.role.uppercase()) }.getOrDefault(Role.USER)
        val now   = LocalDateTime.now()
        val newId = UUID.randomUUID()  // UUID manuell generieren, da Users kein IdTable ist
    ...
    fun login(req: LoginRequest): AuthResponse = transaction {
        val row = Users.select { Users.email eq req.email }.singleOrNull()
            ?: error("INVALID_CREDENTIALS")

        if (!BCrypt.checkpw(req.password, row[Users.passwordHash]))
            error("INVALID_CREDENTIALS")

        // Users.id ist ein Column<UUID>, kein EntityID → direkt auslesen
    ...
    // ── Refresh Token ─────────────────────────────────────
    fun refresh(req: RefreshRequest): RefreshResponse = transaction {
        val row = RefreshTokens.select { RefreshTokens.token eq req.refreshToken }
        ...
        // RefreshTokens.userId ist Column<UUID> → direkt auslesen
        val userId   = row[RefreshTokens.userId]
        val userRole = Users.select { Users.id eq userId }
            .single()[Users.role].name
    ...
    // ── Forgot Password ───────────────────────────────────
    fun createPasswordResetToken(email: String): Pair<String, String>? = transaction {
        val row = Users.select { Users.email eq email }.singleOrNull()
            ?: return@transaction null

        val userId     = row[Users.id]   // Column<UUID> → direkt
    ...
    fun resetPassword(token: String, newPassword: String) = transaction {
        val row = PasswordResets.select { PasswordResets.token eq token }
        ...
        val userId = row[PasswordResets.userId]  // Column<UUID> → direkt
    ...
    // ── Email Verification ────────────────────────────────
    fun createVerificationToken(userId: UUID): String = transaction {
    ...
    fun verifyEmail(token: String) = transaction {
        val row = EmailVerifications.select { EmailVerifications.token eq token }
            .singleOrNull() ?: error("INVALID_TOKEN")
    ...
    fun resendVerification(email: String): Pair<UUID, String>? = transaction {
    ...
    // ── Helpers ───────────────────────────────────────────
    private fun generateAccessToken(userId: UUID, role: String): String =
        JWT.create()
            .withIssuer(issuer)
